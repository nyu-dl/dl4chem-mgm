...
def generate_smiles(params, model, dico, number_samples):
    gen_smiles = []
    while len(gen_smiles) < number_samples:
        # run generation of molecules ( regular sampling)
        with torch.no_grad():
            gen, gen_len = model.generate_unconditional(params, max_len=params.max_len, bs=params.batch_size, sample_temperature=1.0)
